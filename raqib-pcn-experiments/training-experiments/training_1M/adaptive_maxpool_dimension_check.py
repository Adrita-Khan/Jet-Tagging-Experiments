...
# Classifier class WITH DIMENSION PRINTING - UPDATED TO MATCH v3_pool copy.py
class Classifier(torch.nn.Module):
    ...
    def forward(self, x):
        print(f"\n=== CLASSIFIER FORWARD PASS - UPDATED ARCHITECTURE ===")
        print(f"Input to classifier: {x.shape}")  # Expected: [batch_size, 4, 64]
        
        # Apply 1D convolutions along feature dimension
        x = F.relu(self.conv1d(x))
        print(f"After conv1d + ReLU: {x.shape}")  # Expected: [batch_size, 4, 64]
        
        x = F.relu(self.conv1d_2(x))
        print(f"After conv1d_2 + ReLU: {x.shape}")  # Expected: [batch_size, 4, 64]
        
        # Apply max pooling across channels (graph types) to get [batch_size, 64]
        x = torch.max(x, dim=1)[0]
        print(f"After torch.max(x, dim=1)[0] pooling: {x.shape}")  # Expected: [batch_size, 64]
        
        # Final classification - single FC layer
        x = self.fc(x)
...
if __name__ == "__main__":
    print("=== DIMENSION CHECKING SCRIPT ===")
    ...
    print("\n" + "="*80)
    print("STARTING DIMENSION CHECK - FORWARD PASS")
    print("="*80)
    ...
    with torch.no_grad():
        for batch_idx, (graphs, labels) in enumerate(testLoader):
            ...
            print(f"After torch.stack (concatenated_features): {concatenated_features.shape}")
