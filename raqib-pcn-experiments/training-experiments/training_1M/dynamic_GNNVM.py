...
def to_m2(part_i, part_j, eps=1e-8):
    ...
    m2 = (energy_i + energy_j).square() - (p_i + p_j).square().sum()
    if eps is not None:
        m2 = m2.clamp(min=eps)
...
def compute_edge_weights(base_graph, points, weight_type):
    weight_functions = {
        'delta': delta_weight,
        'kT': kT_weight,
        'Z': Z_weight,
    ...
    # Compute weights only for existing edges
    edge_weights = []
    for src, dst in zip(src_nodes, dst_nodes):
        weight = weight_func(points[src], points[dst])
...
def create_weighted_graphs(base_graph, points, weight_type):
    # Get edge weights for the existing graph structure
    edge_weights = compute_edge_weights(base_graph, points, weight_type)
...
class MultiGraphDataset(dgl.data.DGLDataset):
    def __init__(self, jetNames, k, loadFromDisk=False):
        ...
        for jetType in tqdm(jetNames, total=len(jetNames)):
            if type(jetType) != list:
                if loadFromDisk:
                    base_path = f'pickleFiles/{jetType}.pkl'        # base graph unweighted
                else:
                    base_path = f'data/{jetType}.pkl'               # base graph unweighted
    ...
    def __getitem__(self, idx):
        ...
        # Generate 4 different graphs using the same structure with different edge weights
        graph_delta = create_weighted_graphs(base_graph, points, 'delta')
        graph_kT = create_weighted_graphs(base_graph, points, 'kT')
        graph_Z = create_weighted_graphs(base_graph, points, 'Z')
        graph_mSquare = create_weighted_graphs(base_graph, points, 'mSquare')

        # Add node features to all graphs
        node_features = base_graph.ndata['feat'].clone()        # use clone() to ensure same tensor
