...
parser.add_argument('--classification_level', type=str, default='All_Interactions_with_one', help='Classification level (default: All_Interactions_with_one)')
...
class BatchedMultiGraphDataset(dgl.data.DGLDataset):
    def __init__(self, jetNames, k, batchDir='batches', loadFromDisk=False):
        ...
        # Graph types we need to load (file paths stay as 'default' but model is 'one')
        self.graph_types = ['delta', 'kT', 'mSquare', 'Z', 'default']  
        ...
        # Each entry contains paths to all 5 graph types for the same batch
...
def save_intermediate_results(imageSavePath, cfs, jetNames, classificationLevel, modelArchitecture, 
    ...
    try:
        ...
        ax.set_title(f'{classificationLevel} {modelArchitecture} 5-Graph Confusion Matrix (Batch Sets: {file_count}/{total_files})')
...
# Custom collate function for multiple graphs (updated for 5 graphs)
def collateFunction(batch):
    ...
    graphs_one = [item['graph_one'] for item in batch]  # model_one gets data from 'default' files
    ...
    return (batched_graph_delta, batched_graph_kT, batched_graph_mSquare, batched_graph_Z, batched_graph_one), torch.tensor(labels)
...
# Checkpoint file
checkpoint_file = f"checkpoints/{classificationLevel}-{modelArchitecture}-5graph-checkpoint.json"
...
wandb.init(
    ...
    name=f"{classificationLevel}-{modelArchitecture}-5Graph-Testing",
    config={
        "epochs": maxEpochs,
        ...
        "device": device,
        "testing_mode": True,
        "graph_types": ["delta", "kT", "mSquare", "Z", "one"]  # Model names: one, file paths: default
    }
)

# Initialize multi-graph models
if modelType == "DGCNN":
    # Create 5 feature extractors for each graph type
    ...
    model_one = GNNFeatureExtractor(in_feats, hidden_feats, chebFilterSize)
    
    # Final classifier that takes concatenated features
    classifier = Classifier(hidden_feats * 5, hidden_feats, out_feats)
else:
...
all_models = [model_delta, model_kT, model_mSquare, model_Z, model_one, classifier]
...
if load:
    ...
    if isinstance(checkpoint, dict) and 'model_delta' in checkpoint:
        ...
        model_one.load_state_dict(checkpoint['model_one'])  # loads from 'model_one' key
        classifier.load_state_dict(checkpoint['classifier'])
        print(f"Loaded 5-graph model from {modelSaveFile}")
    else:
        print("Error: Model file doesn't contain 5-graph architecture!")
        exit()
...
if args.output_dir:
    ...
    imageSavePath = f'{classificationLevel} {modelArchitecture} 5Graph'
try:
    os.makedirs(imageSavePath, exist_ok=True)
except Exception as e:
    print(e)

print("Starting 5-graph batch-wise testing...")
...
with torch.no_grad():
    for file_idx, (batch_paths, label) in enumerate(tqdm(all_batch_files, desc="Processing batch file sets")):
        ...
        try:
            ...
            # Load all 5 graph types for this batch
            batch_graphs = {}
            batch_size = None
            
            for graph_type in ['delta', 'kT', 'mSquare', 'Z', 'default']:
            ...
            if len(batch_graphs) != 5:
                print(f"Error: Could not load all 5 graph types for batch {file_idx}")
                continue
            ...
            for i in range(batch_size):
                batch_data.append({
                    ...
                    'graph_one': batch_graphs['default'][i],  # model_one gets data from 'default' files
            ...
            for mini_batch_graphs, mini_batch_labels in tqdm(batch_loader, 
                                                           ...
                                                           leave=False):
                # Unpack graphs
                graph_delta, graph_kT, graph_mSquare, graph_Z, graph_one = mini_batch_graphs
                ...
                hg_one = model_one(graph_one)
                
                # Concatenate embeddings
                concatenated_features = torch.cat([hg_delta, hg_kT, hg_mSquare, hg_Z, hg_one], dim=1)
                ...
                # Clean up GPU memory
                del graph_delta, graph_kT, graph_mSquare, graph_Z, graph_one
                del hg_delta, hg_kT, hg_mSquare, hg_Z, hg_one, concatenated_features
...
print("5-graph testing completed!")
...
logitsTrackerFile = f'metrics/{classificationLevel}-{modelArchitecture}-5Graph-Logits.pkl'
targetsTrackerFile = f'metrics/{classificationLevel}-{modelArchitecture}-5Graph-Targets.pkl'
predictionsTrackerFile = f'metrics/{classificationLevel}-{modelArchitecture}-5Graph-Predictions.pkl'
...
ax = sns.heatmap(cfs/np.sum(cfs), annot=True, cmap='Blues')
ax.set_title(f'{classificationLevel} {modelArchitecture} 5-Graph Confusion Matrix')
ax.set_xlabel('Actual Values')
ax.set_ylabel('Predicted Values')
...
try:
    ...
    skplt.metrics.plot_roc_curve(rocTargets, rocLogits, figsize=(8, 6), 
                               title=f'{classificationLevel} {modelArchitecture} 5-Graph ROC-AUC Curve')
    ...
    plt.title(f'{classificationLevel} {modelArchitecture} 5-Graph ROC-AUC Curve (Error)')
...
print("\n" + "="*80)
print("5-GRAPH TESTING COMPLETED SUCCESSFULLY!")
print("="*80)
...
print("5-graph analysis complete!")