...
parser.add_argument('--classification_level', type=str, default='All_Interactions_with_one', help='Classification level (default: All)')
...
class MultiGraphDataset(dgl.data.DGLDataset):
    def __init__(self, jetNames, k, loadFromDisk=False):
        ...
        # Initialize lists for each graph type (WITH 5 GRAPHS)
        self.graphs_delta = []
        self.graphs_kT = []
        self.graphs_mSquare = []
        self.graphs_Z = []
        ...
        for jetType in tqdm(jetNames, total=len(jetNames)):
            if type(jetType) != list:
                ...
                if loadFromDisk:
                    ...
                    one_path = f'pickleFiles/one_{jetType}.pkl'  
                else:
                    # Load from your data directory
                    delta_path = f'data/delta/delta_{jetType}.pkl'
                    kT_path = f'data/kT/kT_{jetType}.pkl'
                    mSquare_path = f'data/mSquare/mSquare_{jetType}.pkl'
                    Z_path = f'data/Z/Z_{jetType}.pkl'
                    one_path = f'data/one/one_{jetType}.pkl'  
                ...
                with open(one_path, 'rb') as f:
                    one_graphs = pickle.load(f)
                    self.graphs_one += one_graphs
                
                # Assuming all graph types have the same number of samples
                self.sampleCountPerClass.append(len(delta_graphs))
                
                del delta_graphs, kT_graphs, mSquare_graphs, Z_graphs, one_graphs  
...
# collate function for multiple graphs (WITH 5 GRAPHS)
def collateFunction(batch):
    ...
    graphs_one = [item['graph_one'] for item in batch]  
...
# Start wandb logging
wandb.init(
    project="All Interaction Vectors 5 Graphs", 
...
for epoch in range(maxEpochs):
    ...
    for batchIndex, (graphs, labels) in tqdm(enumerate(trainLoader), total=len(trainLoader), leave=False):
        # Unpack graphs 
        graph_delta, graph_kT, graph_mSquare, graph_Z, graph_one = graphs  
...
logitsTrackerFile = f'metrics/{classificationLevel}-{modelArchitecture}-5Graphs-Logits.pkl' 
targetsTrackerFile = f'metrics/{classificationLevel}-{modelArchitecture}-5Graphs-Targets.pkl'  
predictionsTrackerFile = f'metrics/{classificationLevel}-{modelArchitecture}-5Graphs-Predictions.pkl' 
...
skplt.metrics.plot_roc_curve(rocTargets, rocLogits, figsize=(8, 6), title=f'{classificationLevel} {modelArchitecture} 5-Graph ROC-AUC Curve')  
