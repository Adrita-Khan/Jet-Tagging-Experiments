    ...
    for chunk_start in range(0, n_jets, chunk_size):
        ...
        for jet in tqdm(range(chunk_start, chunk_end), total=chunk_end-chunk_start):
            ...
            try:
                ...
                px = ak.to_numpy(currJet['part_px'])
                py = ak.to_numpy(currJet['part_py'])
                pz = ak.to_numpy(currJet['part_pz'])
                ...
                # Free memory
                del px, py, pz, energy, pT, currJet_array
...
def fileToPointCloudArray(jetType, input_features):
    filepath = f'merged_test_roots/JetClass/JetRoots/{jetType}-Testing/{jetType}-Testing.root' # original root file
    savepath = f'merged_test_roots/JetClass/PointClouds/{jetType}.npy' # save file
...
def find_last_batch(jetType):
    batch_files = []
    dir_path = 'merged_test_roots/Multi Level Jet Tagging'
    if os.path.exists(dir_path):
        batch_files = [f for f in os.listdir(dir_path) if f.startswith(f"{jetType}_batch_") and f.endswith(".pkl")]
...
def fileToGraph(jetType, k=3, save=True, batch_size=100_000, start_from=None):
    ...
    ensure_dir('merged_test_roots/Multi Level Jet Tagging')
    
    # Set up final save path
    saveFilePath = f'merged_test_roots/Multi Level Jet Tagging/{jetType}.pkl'
    ...
    for batch_start in range(start_from, total_jets, batch_size):
        ...
        if save and batch_graphs:  # Only save if there are graphs in the batch
            temp_save_path = f'merged_test_roots/Multi Level Jet Tagging/{jetType}_batch_{batch_start}_{batch_end}.pkl'
...
def groupToGraph(jetTypeList, groupName):
    ...
    saveFilePath = f'merged_test_roots/Multi Level Jet Tagging/{groupName}.pkl'
...
def merge_batches(jetType):
    ...
    batch_files = sorted([os.path.join(dir_path, f) for f in os.listdir(dir_path) 
                         if f.startswith(f"{jetType}_batch_") and f.endswith(".pkl")])
    ...
    # Save merged file
    final_path = f'merged_test_roots/Multi Level Jet Tagging/{jetType}.pkl'
