...
def saveAdj_Matrices(adj_matrices_list, jetType):
    ...
    os.makedirs(f'../adj_matrices/Z/Z_{jetType}', exist_ok=True)
    
    for idx, adj_matrix in enumerate(adj_matrices_list):
        # File paths
        csv_path = f'../adj_matrices/Z/Z_{jetType}/Z_Emitter-{jetType}_jet_{idx}.csv'
...
for key, value in jet_types.items():
    ...
    with open(f'../data/Multi Level Jet Tagging/Z/Z_Emitter-{key}.pkl', 'wb') as f:
