...
def saveAdj_Matrices(adj_matrices_list, jetType):
    ...
    os.makedirs(f'../adj_matrices/mSquare/mSquare_{jetType}', exist_ok=True)
    
    for idx, adj_matrix in enumerate(adj_matrices_list):
        # File paths
        csv_path = f'../adj_matrices/mSquare/mSquare_{jetType}/mSquare_Emitter-{jetType}_jet_{idx}.csv'
...
def groupToGraph(jetTypeList, groupName, label):
    ...
    saveFilePath = f'../data/Multi Level Jet Tagging/mSquare/mSquare_{groupName}.pkl'
    with open(saveFilePath, 'wb') as f:
...
for key, value in jet_types.items():
    ...
    with open(f'../data/Multi Level Jet Tagging/mSquare/mSquare_Emitter-{key}.pkl', 'wb') as f:
