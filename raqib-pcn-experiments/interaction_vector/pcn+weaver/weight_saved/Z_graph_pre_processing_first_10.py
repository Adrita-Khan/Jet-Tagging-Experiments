...
def fileToGraph(jetType, k=3, save=True):
    ...
    saveFilePath = f'weighted_graphs/Z/Z_{jetType}_first_10.pkl'

    # Create directory for adjacency matrices CSV files
    adj_csv_dir = f'adj_matrices/Z/{jetType}'
...
def groupToGraph(jetTypeList, groupName):
    ...
    saveFilePath = f'weighted_graphs/Z/Z_{groupName}_first_10.pkl' 
    return allGraphs
...
pd.DataFrame(missing_graphs).to_csv(os.path.join(log_dir, 'missing_graphs_z.csv'), index=True)
