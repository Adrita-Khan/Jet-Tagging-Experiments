...
def mSquare(part_i, part_j, eps=1e-8):
...
def buildKNNGraph(points, k):
    ...
    for i in range(num_points):
        for j in indices[i, 1:]:  # exclude self
            weight_m_square = mSquare(points[i], points[j])
...
def adjacencyToDGL(adj_matrix):
    ...
    g_dgl.edata['weight'] = torch.tensor(adj_matrix.data, dtype=torch.float32)
...
def fileToPointCloudArray(jetType, input_features):
    ...
    nparr = awkToPointCloud(awk[:10], input_features)
...
def fileToGraph(jetType, k=3, save=True):
    ...
    saveFilePath = f'weighted_graphs/mSquare/mSquare_{jetType}_first_10.pkl'

    # Create directory for adjacency matrices CSV files
    adj_csv_dir = f'adj_matrices/mSquare/{jetType}'
    os.makedirs(adj_csv_dir, exist_ok=True)
    
    savedGraphs = []
    for idx, pointCloud in tqdm(enumerate(pointCloudArr[:10]), leave=False, total=10):
        try:
...
def groupToGraph(jetTypeList, groupName):
    ...
    saveFilePath = f'weighted_graphs/mSquare/mSquare_{groupName}_first_10.pkl' 
    return allGraphs
...
pd.DataFrame(missing_graphs).to_csv(os.path.join(log_dir, 'missing_graphs_mSquare.csv'), index=True)
