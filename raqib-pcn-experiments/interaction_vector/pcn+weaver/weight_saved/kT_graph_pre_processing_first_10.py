...
def fileToGraph(jetType, k=3, save=True):
    ...
    saveFilePath = f'weighted_graphs/kT/kT_{jetType}_first_10.pkl'
...
def groupToGraph(jetTypeList, groupName):
    ...
    saveFilePath = f'weighted_graphs/kT/kT_{groupName}_first_10.pkl' 
    return allGraphs
...
pd.DataFrame(missing_graphs).to_csv(os.path.join(log_dir, 'missing_graphs_kT.csv'), index=True)
