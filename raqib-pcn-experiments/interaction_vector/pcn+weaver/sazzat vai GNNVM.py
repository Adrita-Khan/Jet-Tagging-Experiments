...
class GraphDataset(dgl.data.DGLDataset):
    ...
    def __getitem__(self, idx):
        ...
        return {'graph': (self.graphs1[idx]), 'label': self.labels[idx]}
...
#         # do 1d conv 
#         # self.fc = nn.Linear(-----, out_feats)

#     def forward(self, hg):

#         # logits = self.fc(self.1dconv(hg))
...
if modelType == "GCNN":
  model1 = GNNClassifier(in_feats, hidden_feats, out_feats, chebFilterSize)
  model2 = GNNClassifier(in_feats, hidden_feats, out_feats, chebFilterSize)
  model3 = GNNClassifier(in_feats, hidden_feats, out_feats, chebFilterSize)
  model4 = GNNClassifier(in_feats, hidden_feats, out_feats, chebFilterSize)
  fcl = Classifier(in_feats, hidden_feats, out_feats)
...
for epoch in range(maxEpochs):
    ...
    for batchIndex, (graph, labels) in tqdm(enumerate(trainLoader), total=len(trainLoader), leave=False):
        ...
        # concat 4 vectors horizontally
        # create another model which takes the new vector and returns logits
